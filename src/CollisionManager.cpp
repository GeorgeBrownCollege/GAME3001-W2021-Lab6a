...
// assumptions - the list of objects are sorted so that they are facing the target and the target is loaded last
bool CollisionManager::LOSCheck(glm::vec2 start_point, glm::vec2 end_point,	const std::vector<DisplayObject*>& game_objects, DisplayObject* target)
{
	for (auto object : game_objects)
	{

			// check if Line collides with an object in the list
			if (lineRectCheck(start_point, end_point, object->getTransform()->position - glm::vec2(object->getWidth() * 0.5f,
				object->getHeight() * 0.5f), object->getWidth(), object->getHeight()))
			{				
				// if the collison is with the target object then LOS is true
				...
				// if the line collides with an object in the list that is not the target then no LOS
				return false;
			}
	...
	// if the line does not collide with an object that is the target then no LOS
	return false;
}
